                        ...
                        if antecedent_support > 0:
                            ...
                            if confidence >= self.min_confidence:
                                ...
                                if len(consequent) == 1:
                                    ...
                                    consequent_support = self.frequent_itemsets.get(
                                        len(consequent), {}
                                    ).get(consequent, 0)
    ...
    def save_segments_to_db(self):
        ...
        for _, row in self.segments.iterrows():
            CustomerSegment.objects.create(
                ...
                avg_basket_value=(
                    row["monetary"] / row["frequency"] if row["frequency"] > 0 else 0
                ),
            )
...
def _legacy_build_churn_feature_set(prediction_point_offset=30):
    ...
    avg_purchase_gap = (
        temp_df.groupby("household_key")["purchase_gap"].mean().reset_index()
    )
    avg_purchase_gap.rename(columns={"purchase_gap": "avg_purchase_gap"}, inplace=True)

    product_variety = (
        history_df.groupby("household_key")["product_id"].nunique().reset_index()
    )
