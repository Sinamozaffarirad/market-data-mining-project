...
def _write_cached_training_dataset(cache, dataset):
    if cache.training_dataset_blob:
        return
    cache.training_dataset_blob = gzip.compress(
...
def _read_cached_model_result(cache):
    ...
    try:
        ...
        historical_predictions = pickle.loads(
            gzip.decompress(bytes(cache.historical_predictions_blob))
        )
    except (
        OSError,
        EOFError,
        ...
        logger.warning(
            "Ignoring an unreadable churn model-result cache (id=%s).", cache.pk
        )
        return None
...
def _write_cached_model_result(cache, metrics, scores, historical_predictions):
    ...
    cache.current_scores_blob = gzip.compress(
        pickle.dumps(scores, protocol=pickle.HIGHEST_PROTOCOL)
    )
    ...
    cache.save(
        update_fields=[
            "model_result_signature",
            "model_metrics_json",
            "current_scores_blob",
            "historical_predictions_blob",
            "model_result_cached_at",
        ]
    )
...
def _get_or_build_window_cache(
    ...
    if created or not cache.customer_windows.exists():
        snapshots = (
            ...
            else build_history_dataset(config)
        )
        if snapshots.empty:
        ...
        CachedCustomerWindow.objects.bulk_create(
            [
                CachedCustomerWindow(
                    ...
                    observation_start=int(row.observation_start),
                    observation_end=int(row.observation_end),
                    cutoff_day=int(row.cutoff_day),
                    label_start=int(row.label_start),
                    label_end=int(row.label_end),
    ...
    return cache, {
        "window_records": cache.customer_windows.count(),
        "cache_reused": not created,
    }
...
def _persist_cache_predictions(experiment, cache, historical_predictions):
    ...
    window_ids = {
        ...
        for household, cutoff, pk in CachedCustomerWindow.objects.filter(
            cache=cache
        ).values_list("household_key", "cutoff_day", "pk")
    }
    predictions = [
        ChurnExperimentWindowPrediction(
...
def _persist_window_history(experiment, historical_snapshots, historical_predictions):
    ...
    probability_by_window = {
        (
            ...
            int(row.cutoff_day),
            int(row.observation_window_days),
    ...
    return {
        ...
        "scored_records": sum(
            record.churn_probability is not None for record in records
        ),
        "outcomes": len(records),
    }


def _persist_churn_history(
    experiment,
    historical_snapshots,
    ...
    unique_states = states.drop_duplicates(
        ["household_key", "cutoff_day", "observation_window_days"]
    )
    CustomerStateSnapshot.objects.bulk_create(
        [
            CustomerStateSnapshot(
                household_key=int(row.household_key),
                cutoff_day=int(row.cutoff_day),
                observation_window_days=int(row.observation_window_days),
                recency_days=float(row.recency_days),
                frequency=float(row.frequency),
                monetary=float(row.monetary),
                r_score=int(row.r_score),
    ...
    lookup = {
        (household, cutoff, observation): pk
        for household, cutoff, observation, pk in CustomerStateSnapshot.objects.filter(
    ...
    if missing_keys:
        raise RuntimeError(
            f"Could not save {len(missing_keys)} customer RFM snapshots."
        )
    ...
    score_map = dict(
        zip(
            current_scores["household_key"].astype(int),
            current_scores["churn_probability"].astype(float),
        )
    )
    CustomerChurnPrediction.objects.bulk_create(
        [
            CustomerChurnPrediction(
                ...
                churn_probability=float(score_map[int(row.household_key)]),
                prediction_type=CustomerChurnPrediction.CURRENT,
            )
    ...
    historical_count = experiment.history_predictions.filter(
        prediction_type=CustomerChurnPrediction.HISTORICAL
    ).count()
    current_count = experiment.history_predictions.filter(
        prediction_type=CustomerChurnPrediction.CURRENT
    ).count()
    return {
        "snapshots": len(desired_keys),
        "outcomes": int(
            historical_snapshots.drop_duplicates(
                ["household_key", "cutoff_day", "observation_window_days"]
            ).shape[0]
        ),
...
def admin_required(view_func):
    ...
    decorated_view = user_passes_test(check_admin, login_url="/analysis/login/")(
        view_func
    )
...
def refresh_basket_analysis_logic():
    ...
    with db_transaction.atomic():
        ...
        baskets = (
            Transaction.objects.values("basket_id", "household_key")
            .annotate(total_items=Sum("quantity"), total_value=Sum("sales_value"))
            .iterator()
        )
...
def api_refresh_basket_analysis(request):
    try:
        ...
        return JsonResponse(
            {
                "success": True,
                "message": f"Basket analysis refreshed successfully. {count} records processed.",
            }
        )
    ...
    try:
        ...
        if transaction_period != "all":
            ...
            with connection.cursor() as cursor:
                ...
                logger.info(
                    f"Filtering transactions from day {start_day} to {max_day} ({transaction_period})"
                )
        ...
        with connection.cursor() as cursor:
            ...
            if start_day is not None:
                ...
                basket_count_query = (
                    "SELECT COUNT(DISTINCT basket_id) FROM transactions"
                )
        ...
        for product_a, product_b, pair_count, count_a, count_b in product_pairs:
            ...
            if confidence_a_to_b >= min_confidence:
                ...
                ant_detail = product_details.get(
                    str(product_a),
                    {
                        "department": "GENERAL",
                        "commodity": f"Product {product_a}",
                        "brand": "Generic",
                        "size": "N/A",
                    },
                )
                cons_detail = product_details.get(
                    str(product_b),
                    {
                        "department": "GENERAL",
                        "commodity": f"Product {product_b}",
                        "brand": "Generic",
                        "size": "N/A",
                    },
                )
            ...
            if (
                ...
                lift = (
                    confidence_b_to_a / (count_a / total_baskets) if count_a > 0 else 0
                )
    ...
    try:
        ...
        if transaction_period != "all":
            ...
            if max_day is not None:
                ...
                start_day = max(1, max_day - days_back + 1)
                logger.info(
                    "Filtering department transactions from day %s to %s (%s)",
                    start_day,
                    max_day,
                    transaction_period,
                )
        ...
        with connection.cursor() as cursor:
            if start_day is not None:
                cursor.execute(
                    "SELECT COUNT(DISTINCT basket_id) FROM transactions WHERE day >= %s",
                    (start_day,),
                )
            else:
        ...
        logger.info(
            "Department rules: total baskets=%s, min basket count=%s",
            total_baskets,
            min_basket_count,
        )
        ...
        if not eligible_rows or len(eligible_rows) < 2:
            logger.info(
                "No departments met the minimum support threshold (%s)", min_support
            )
            return rules
        ...
        if not pair_counts:
            logger.info(
                "No department pairs met qualification criteria after streaming"
            )
            return rules
        ...
        all_rules = sorted(rules, key=lambda x: x["lift"], reverse=True)
        logger.info(
            "Generated %s department rules after streaming, returning top %s",
            len(all_rules),
            max_results,
        )
        return all_rules[:max_results]
    ...
    try:
        ...
        if transaction_period != "all":
            ...
            if max_day is not None:
                ...
                start_day = max(1, max_day - days_back + 1)
                logger.info(
                    "Filtering commodity transactions from day %s to %s (%s)",
                    start_day,
                    max_day,
                    transaction_period,
                )
        ...
        min_basket_count = max(1, int(total_baskets * min_support))
        logger.info(
            "Commodity rules: total baskets=%s, min basket count=%s",
            total_baskets,
            min_basket_count,
        )
        ...
        if not eligible_rows or len(eligible_rows) < 2:
            logger.info(
                "No commodities met the minimum support threshold (%s)", min_support
            )
            return rules
        ...
        for (comm_a, comm_b), pair_count in pair_counts.items():
            ...
            if confidence_a_to_b >= min_confidence:
                lift = confidence_a_to_b / (
                    commodity_counts.get(comm_b, 1) / total_baskets
                )
                rules.append(
                    {
                        "antecedent": comm_a,
            ...
            if (
                ...
                lift = confidence_b_to_a / (
                    commodity_counts.get(comm_a, 1) / total_baskets
                )
                rules.append(
                    {
                        "antecedent": comm_b,
        ...
        all_rules = sorted(rules, key=lambda x: x["lift"], reverse=True)
        logger.info(
            "Generated %s commodity rules after streaming, returning top %s",
            len(all_rules),
            max_results,
        )
        return all_rules[:max_results]
...
def site_index(request):
    ...
    recent_products = (
        Transaction.objects.filter(day__gte=recent_period_start)
        .values("product_id")
        .distinct()
        .count()
    )
    ...
    recent_revenue = (
        Transaction.objects.filter(day__gte=recent_period_start).aggregate(
    ...
    prev_revenue = (
        Transaction.objects.filter(
            day__gte=previous_period_start, day__lte=previous_period_end
        ).aggregate(total=Sum("sales_value"))["total"]
        or 0
    )
    ...
    prod_change = (
        ((recent_products - prev_products) / prev_products * 100)
        if prev_products > 0
        else 0
    )
    cust_change = (
        ((recent_customers - prev_customers) / prev_customers * 100)
        if prev_customers > 0
        else 0
    )
    rev_change = (
        ((float(recent_revenue) - float(prev_revenue)) / float(prev_revenue) * 100)
        if prev_revenue > 0
        else 0
    )
    ...
    tools = [
        {
            ...
            "description": "Analyze baskets, top products, and patterns",
            "url": "basket-analysis/",
            "icon": '<i class="fas fa-shopping-basket"></i>',
            "action": "Explore baskets",
            "action_icon": '<i class="fas fa-magnifying-glass-chart"></i>',
        },
        {
            "title": "Association Rules",
            "description": "Market basket association rules",
            "url": "association-rules/",
            "icon": '<i class="fas fa-project-diagram"></i>',
            "action": "Browse rules",
            "action_icon": '<i class="fas fa-diagram-project"></i>',
        },
        {
            "title": "Customer Segments",
            "description": "RFM segments & behavior",
            "url": "customer-segments/",
            "icon": '<i class="fas fa-users"></i>',
            "action": "View segments",
            "action_icon": '<i class="fas fa-chart-pie"></i>',
        },
        {
            "title": "Customer Retention",
            "description": "Identify at-risk customers and predict churn",
            "url": "customer-retention/",
            "icon": '<i class="fas fa-heart-circle-check"></i>',
            "action": "Find at-risk buyers",
            "action_icon": '<i class="fas fa-user-clock"></i>',
        },
        {
            "title": "Data Management",
            "description": "View, edit, import/export data",
            "url": "data-management/",
            "icon": '<i class="fas fa-database"></i>',
            "action": "Manage data",
            "action_icon": '<i class="fas fa-table"></i>',
        },
        {
            "title": "Customer Insights",
            "description": "Explore and manage your customer data in detail",
            "url": reverse("customers:search"),
            "icon": '<i class="fas fa-user"></i>',
            "action": "Search customers",
            "action_icon": '<i class="fas fa-magnifying-glass"></i>',
        },
        {
            "title": "Product Recommender",
            "description": "Find customers most likely to buy selected product",
            "url": "product-recommender/",
            "icon": '<i class="fas fa-file-pen"></i>',
            "action": "Find likely buyers",
            "action_icon": '<i class="fas fa-bullseye"></i>',
        },
    ]
...
def api_regenerate_segments(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": f"{str(e)} - Check server logs for details"},
            status=500,
        )
...
def basket_analysis(request):
    ...
    try:
        ...
        top_products_frequency_raw = list(
            product_stats.order_by("-frequency")[:top_products_size]
        )
        top_products_sales_raw = list(
        ...
        product_details = {
            ...
            for item in DunnhumbyProduct.objects.filter(product_id__in=all_product_ids)
        }
                ...
                enriched.append(
                    {
                        ...
                        "brand": (
                            product_detail.brand if product_detail else "Unknown Brand"
                        ),
                        "department": (
                            product_detail.department
                            if product_detail
                            else "MISC. TRANS."
                        ),
                        "commodity_desc": (
                            product_detail.commodity_desc
                            if product_detail
                            else "Unknown Product"
                        ),
                        "sub_commodity_desc": (
                            product_detail.sub_commodity_desc if product_detail else ""
                        ),
                        "manufacturer": (
                            product_detail.manufacturer if product_detail else 0
                        ),
                    }
                )
            return enriched
...
def _bi_filter_options():
    ...
    try:
        ...
        commodities = [
            ...
            for r in rows(
                "SELECT DISTINCT commodity, department FROM vw_dim_product "
        ...
        return {
            ...
            "stores": stores,
            "incomes": banded(
                "income", "SELECT DISTINCT income_band FROM vw_dim_household"
            ),
...
def _describe_stored_rules(rules):
    ...
    with connection.cursor() as cursor:
        if product_ids:
            ...
            if ids:
                for pid, department, commodity, brand in _rows_in_batches(
                    cursor,
                    """
        ...
        if commodities:
            for commodity, department in _rows_in_batches(
                cursor,
                """
    ...
    def describe(value, rule_type):
        ...
        if rule_type == "product":
            return products.get(
                ...
                {
                    "department": "UNKNOWN",
                    "commodity": f"Product {text}",
                    "brand": "Generic",
                },
            )
        if rule_type == "commodity":
            return {
                "department": commodity_departments.get(text, "UNKNOWN"),
                "commodity": text,
                "brand": "",
            }
        ...
        described.append(
            {
                ...
                "antecedent_details": [
                    describe(v, rule.rule_type) for v in (rule.antecedent or [])
                ],
                "consequent_details": [
...
def _mark_saved_state(rules):
    ...
    for row in AssociationRule.objects.filter(rule_type__in=wanted):
        ...
        stored[(key_part(row.antecedent), key_part(row.consequent), row.rule_type)] = (
            row
        )
    ...
    if request.method == "POST":
        try:
            ...
            if "timeout" in error_msg.lower():
                ...
                suggestion = (
                    "Database syntax error. Please try again or contact support."
                )
            else:
...
def data_management(request):
    return render(
        request,
        "site/dunnhumby/data_management.html",
        {
...
def api_insert_association_rule(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": "Invalid numeric values supplied."}, status=400
        )

    if not antecedent or not consequent:
        return JsonResponse(
            {"success": False, "error": "Antecedent and consequent are required."},
            status=400,
        )
    ...
    min_support = _parse_float(
        payload.get("min_support_threshold") or payload.get("min_support"), support
    )
    min_confidence = _parse_float(
        payload.get("min_confidence_threshold") or payload.get("min_confidence")
    )
    min_lift = _parse_float(
        payload.get("min_lift_threshold") or payload.get("min_lift")
    )
    ...
    existing = AssociationRule.objects.filter(
        antecedent=antecedent, consequent=consequent, rule_type=rule_type
    ).first()
...
def api_get_table_data(request):
    ...
    try:
        ...
        filters = (
            json.loads(request.POST.get("filters", "{}"))
...
def api_basket_details(request):
    ...
    try:
        ...
        prod_info = {}
        if unique_products:
            for p in DunnhumbyProduct.objects.filter(
                product_id__in=unique_products
            ).values("product_id", "brand", "department", "commodity_desc"):
                prod_info[p["product_id"]] = {
                    "brand": p["brand"],
                    "department": p["department"],
                    "commodity_desc": p["commodity_desc"],
                }
        for i in items:
...
def api_product_details(request):
    ...
    try:
        ...
        top_households = list(
            ...
            .values("household_key")
            .annotate(spend=Sum("sales_value"))
            .order_by("-spend")[:5]
        )
        prod = (
            DunnhumbyProduct.objects.filter(product_id=product_id)
            .values("commodity_desc", "brand", "department", "manufacturer")
            .first()
            or {}
        )
        return JsonResponse(
            {
                ...
                "meta": prod,
                "stats": {
                    k: float(v) if v is not None and k == "total_sales" else v
...
def api_household_details(request):
    ...
    try:
        seg = (
            CustomerSegment.objects.filter(household_key=household_key)
            .values(
                "rfm_segment",
                "recency_score",
                "frequency_score",
        ...
        prodmap = {
            ...
            for p in DunnhumbyProduct.objects.filter(product_id__in=pids).values(
...
def api_differential_analysis(request):
    ...
    try:
        ...
        def fetch_top_products_by_department(
            department, start_day, end_day, total_sales, limit=3
        ):
            if not department:
                return []
        ...
        def fetch_basket_totals_for_range(start_day, end_day, limit=2500):
            totals = (
                Transaction.objects.filter(day__gte=start_day, day__lte=end_day)
                .values("basket_id")
                .annotate(total_value=Sum("sales_value"))
                .order_by()[:limit]
            )
            return [
                to_float(item["total_value"])
                for item in totals
        ...
        def compute_statistics(test_name, observed=None, group_a=None, group_b=None):
            ...
            try:
                if (
                    ...
                    if (
                        observed_arr.size
                        and observed_arr.sum() > 0
                        and observed_arr.shape[0] > 1
                        and observed_arr.shape[1] > 1
                    ):
                        ...
                        cohen_w = (
                            sqrt(np.sum((observed_arr - expected) ** 2 / expected) / n)
                            if n > 0
                            else 0.0
                        )
                    ...
                    ks_stat, p_value = ks_2samp(
                        group_a, group_b, alternative="two-sided", mode="auto"
                    )
                    stats["p_value"] = format_stat_value(p_value)
        ...
        def analyze_time_comparison():
            ...
            ordered_quarters = [
                q for q in ["Q1", "Q2", "Q3", "Q4"] if q in quarter_totals
            ]
            department_order = sorted(
                dept_quarters.keys(),
                key=lambda dept: sum(
                    item["sales"] for item in dept_quarters[dept].values()
                ),
                reverse=True,
            )[:6]
            ...
            for dept in department_order:
                ...
                if pct_diff < 12:
                    continue
                top_products = (
                    fetch_top_products_by_department(
                        dept,
                        QUARTER_RANGES[max_quarter][0],
                        QUARTER_RANGES[max_quarter][1],
                        max_sales,
                    )
                    if max_quarter in QUARTER_RANGES
                    else []
                )
                commodity_name = (
                    top_products[0]["name"] if top_products else "Seasonal Demand"
                )
            ...
            insights = [
                ...
                for _, entry in sorted(
                    insight_candidates, key=lambda item: item[0], reverse=True
                )[:4]
            ]
                ...
                "datasets": [
                    {
                        "label": "Total Sales",
                        "data": [
                            round(quarter_totals[q]["sales"], 2)
                    ...
                    },
                    {
                        ...
                        "data": [
                            round(quarter_average.get(q, 0.0), 2)
                            for q in ordered_quarters
                        ],
            ...
            if len(quarter_totals) >= 2:
                ...
                for dept in department_order:
                    ...
                    low_sales = (
                        dept_quarters.get(dept, {}).get(low_quarter, {}).get("sales", 0)
                    )
                    if peak_sales > 0:
        ...
        def analyze_segment_comparison():
            insights = []
            segments = list(
                CustomerSegment.objects.values("rfm_segment")
                .annotate(
            ...
            if len(segments) < 2:
                return (
                    insights,
                    {"p_value": "N/A", "effect_size": "N/A", "confidence": 0},
                    {},
                )
            ...
            segment_departments = defaultdict(
                lambda: defaultdict(lambda: {"sales": 0.0, "count": 0})
            )
            ...
            with connection.cursor() as cursor:
                ...
                for segment, department, sales, txn_count in cursor.fetchall():
                    ...
                    segment_departments[segment][department]["count"] += int(
                        txn_count or 0
                    )
            ...
            spend_diff = (
                ((spend_high - spend_low) / spend_low * 100) if spend_low else 0
            )
            top_products_high = fetch_top_products_for_segment(
                high_seg["rfm_segment"], segment_totals.get(high_seg["rfm_segment"])
            )
            ...
            insights.append(
                {
                    ...
                    "commodity": (
                        top_products_high[0]["name"]
                        if top_products_high
            ...
            if mid_seg:
                ...
                if basket_mid:
                    ...
                    top_products_mid = fetch_top_products_for_segment(
                        mid_seg["rfm_segment"],
                        segment_totals.get(mid_seg["rfm_segment"]),
                    )
                    insights.append(
                        {
                            ...
                            "commodity": (
                                top_products_mid[0]["name"]
                                if top_products_mid
                                else "Basket Optimization"
                            ),
            ...
            if high_values and low_values:
                ...
                frequency_diff = (
                    ((freq_high - freq_low) / freq_low * 100) if freq_low else 0
                )
                insights.append(
                    {
                        ...
                        "top_products": fetch_top_products_for_segment(
                            low_seg["rfm_segment"],
                            segment_totals.get(low_seg["rfm_segment"]),
                        ),
                    }
                )
            ...
            labels = sorted(
                {
                    dept
                    for dept_counts in segment_departments.values()
            ...
            labels = sorted(
                labels,
                key=lambda d: sum(
                    segment_departments[seg][d]["sales"]
                    for seg in segment_departments
                    if d in segment_departments[seg]
                ),
                reverse=True,
            )[:6]
            ...
            for segment in [high_seg["rfm_segment"], low_seg["rfm_segment"]]:
                observed.append(
                    [
                        int(
                            segment_departments[segment].get(label, {}).get("sales", 0)
                            / 1000
                        )
                        for label in labels
                    ]
                ...
                "datasets": [
                    {
                        ...
                        "data": [
                            round(
                                segment_departments[high_seg["rfm_segment"]]
                                .get(label, {})
                                .get("sales", 0.0),
                                2,
                            )
                    ...
                    },
                    {
                        ...
                        "data": [
                            round(
                                segment_departments[low_seg["rfm_segment"]]
                                .get(label, {})
                                .get("sales", 0.0),
                                2,
                            )
        ...
        def analyze_store_comparison():
            ...
            with connection.cursor() as cursor:
                ...
                for store_id, department, sales, txn_count in cursor.fetchall():
                    ...
                    store_departments[store_id][department]["count"] += int(
                        txn_count or 0
                    )
            ...
            top_stores = sorted(
                store_totals.keys(), key=lambda sid: store_totals[sid], reverse=True
            )[:2]
            ...
            best_dept = max(
                store_departments[best_store],
                key=lambda dept: store_departments[best_store][dept]["sales"],
            )
            top_products_best = fetch_top_products_for_store(
            ...
            insights.append(
                {
                    ...
                    "commodity": (
                        top_products_best[0]["name"]
                        if top_products_best
            ...
            total_customers_high = sum(
                entry["count"] for entry in engagement_high.values()
            )
            total_customers_low = sum(
                entry["count"] for entry in engagement_low.values()
            )
            customer_diff = (
                (
                    (total_customers_high - total_customers_low)
                    / total_customers_low
                    * 100
                )
                if total_customers_low
                else 0
            )
            ...
            insights.append(
                {
                    ...
                    "top_products": fetch_top_products_for_store(
                        runner_store, store_totals[runner_store]
                    ),
                }
            )
            ...
            labels = sorted(
                {
                    ...
                    for dept in store_departments[store].keys()
                },
                key=lambda d: sum(
                    store_departments[s][d]["sales"]
                    for s in top_stores
                    if d in store_departments[s]
                ),
                reverse=True,
            )[:6]
            ...
            for store_id in top_stores:
                observed.append(
                    [
                        int(
                            store_departments[store_id].get(label, {}).get("sales", 0)
                            / 1000
                        )
                        for label in labels
                    ]
            ...
            chart = {
                ...
                "datasets": [
                    ...
                    },
                    {
                        ...
                        "data": [
                            round(
                                store_departments[runner_store]
                                .get(label, {})
                                .get("sales", 0.0),
                                2,
                            )
            ...
            for label in labels:
                best_sales = (
                    store_departments[best_store].get(label, {}).get("sales", 0)
                )
                runner_sales = (
                    store_departments[runner_store].get(label, {}).get("sales", 0)
                )
                if best_sales > 0:
        ...
        def analyze_season_comparison():
            ...
            if not top_departments:
                ...
                stats.setdefault(
                    "note", "Not enough data to compare seasonal demand patterns."
                )
            ...
            if not dept_season:
                ...
                stats.setdefault(
                    "note",
                    "Seasonal breakdowns are unavailable for the selected data sample.",
                )
                return insights, stats, {}
            ...
            if not season_order:
                stats = compute_statistics(stat_test)
                stats.setdefault(
                    "note", "Seasonal totals not available for comparison."
                )
                return insights, stats, {}
            ...
            for season in season_order:
                observed.append(
                    [
                        int(
                            dept_season.get(dept, {}).get(season, {}).get("sales", 0)
                            / 1000
                        )
                ...
                "datasets": [
                    {
                        "label": "Total Sales",
                        "data": [
                            round(season_totals[s]["sales"], 2) for s in season_order
                        ],
                    ...
                    },
                    {
                        ...
                        "data": [
                            (
                                round(
                                    season_totals[s]["sales"]
                                    / season_totals[s]["baskets"],
                                    2,
                                )
                                if season_totals[s]["baskets"]
                                else 0
                            )
            ...
            for dept in top_departments_matrix:
                peak_sales = (
                    dept_season.get(dept, {}).get(peak_season, {}).get("sales", 0)
                )
                low_sales = (
                    dept_season.get(dept, {}).get(low_season, {}).get("sales", 0)
                )
                if peak_sales > 0:
                    group_a_sales.append(peak_sales)
                if low_sales > 0:
            ...
            if (
                ...
                and len(season_order) >= 2
                and len(top_departments_matrix) >= 2
            ):
...
def api_create_record(request):
    ...
    if table_name in READ_ONLY_ANALYTICAL_TABLES:
        return JsonResponse(
            {
                "success": False,
                "error": f"Creation is not allowed for the {table_name} table.",
            },
            status=403,
        )
    ...
    if not model:
        return JsonResponse(
            {"success": False, "error": "Unsupported table for creation"}, status=400
        )
...
def api_update_record(request):
    ...
    if table_name in READ_ONLY_ANALYTICAL_TABLES:
        return JsonResponse(
            {
                "success": False,
                "error": f"Updates are not allowed for the {table_name} table.",
            },
            status=403,
        )
    ...
    if not model:
        return JsonResponse(
            {"success": False, "error": "Unsupported table for updates"}, status=400
        )
    ...
    try:
        ...
        if updated_count > 0:
            ...
            return JsonResponse(
                {"success": False, "error": "Record not found or no changes detected."},
                status=404,
            )
...
def api_delete_record(request):
    ...
    if not model:
        return JsonResponse(
            {"success": False, "error": "Deletion from this table is not permitted"},
            status=403,
        )
        ...
        return JsonResponse(
            {
                ...
                "metrics": {
                    ...
                    "total_transactions": total_transactions,
                    "avg_basket_size": (
                        round(avg_basket_size, 1) if avg_basket_size else 0
                    ),
                    "transaction_count_text": transaction_count_text,
                },
                "date_range": {"start_day": start_day, "max_day": max_day},
            }
        )
...
def predictive_analysis_api(request):
    ...
    try:
        ...
        from .repurchase_classifier import (
            VALID_HORIZON_MONTHS,
            horizon_key as build_horizon_key,
        )

        selected_horizon = time_horizon if (time_horizon in VALID_HORIZON_MONTHS) else 3

        department_predictions = ml_analyzer.get_department_predictions(
            model_type, selected_horizon
        )
        ...
        return JsonResponse(
            {
                ...
                "model_metrics": ml_analyzer.get_model_performance().get(
                    f"{horizon_key}_{model_type}", {}
                ),
...
def _time_series_params(request):
    ...
    parameter_mode = source.get("parameter_mode", "auto")
    hidden_units = source.get("hidden_units") or None
    feedback_rate = source.get("feedback_rate") or None
    epochs = source.get("epochs") or None
    return (
        horizon,
        window_size,
        sliding_step,
...
def product_time_series_forecast(request):
    ...
    if request.method not in {"GET", "POST"}:
        return JsonResponse(
            {"success": False, "error": "GET or POST method required"}, status=405
        )
    try:
        (
            horizon,
    ...
    try:
        ...
        if report is None:
            return JsonResponse(
                {
                    "success": False,
                    "error": "No report exists for this horizon/window yet.",
                },
                status=404,
            )
        return JsonResponse({"success": True, "report": report})
...
def train_ml_models(request):
    ...
    if time_horizon_param is not None:
        ...
        try:
            months = int(key)
            horizon_key = (
                build_horizon_key(months) if months in VALID_HORIZON_MONTHS else None
            )
        except ValueError:
        ...
        return JsonResponse(
            {
                ...
                "status": "training",
                "message": ml_training_status.get(
    ...
    if not force_retrain and ml_analyzer.has_cached_models(
        ...
        logger.info(
            "Cached models detected for horizon %s; skipping retraining.",
            horizon_key or "all",
        )
    ...
    def train_models_task():
        ...
        try:
            logger.info(
                "Starting ML training for horizon %s (force=%s, model=%s)",
                horizon_key or "all",
                force_retrain,
                model_type,
            )
...
def get_predictions(request):
    ...
    try:
        predictions = ml_analyzer.get_department_predictions(
            model_type, selected_horizon
        )
        response_horizon = (
            predictions[0].get("time_horizon_months", selected_horizon)
            if predictions
...
def customer_segments(request):
    ...
    segments_list.sort(
        key=lambda s: (
            segment_order.index(s["rfm_segment"])
            if s["rfm_segment"] in segment_order
    ...
    if active_experiment:
        ...
        for risk_label, risk_filter in risk_filters:
            customers = list(
                CustomerSegment.objects.filter(risk_filter).order_by(
                    "-churn_probability", "-total_spend", "household_key"
                )[:5]
            )
            if customers:
    ...
    return render(
        ...
        "site/dunnhumby/customer_segments.html",
        {
            ...
            "lifetime_rfm_cutoff_day": Transaction.objects.aggregate(
                max_day=Max("day")
            )["max_day"],
            "dataset_day_bounds": Transaction.objects.aggregate(
...
def run_churn_experiment(request):
    ...
    try:
        ...
        if observation <= 0 or horizon <= 0:
            raise ValueError(
                "Observation window and prediction horizon must be positive."
            )
        ...
        if restored_from_model_cache:
            ...
            (
                ...
                config,
                cached_training_dataset=cached_training_dataset,
            )
        ...
        with db_transaction.atomic():
            ...
            history_stats["scored_records"] = _persist_cache_predictions(
                experiment, window_cache, historical_predictions
            )
            if replace_active_rule:
                score_map = dict(
                    experiment.scores.values_list("household_key", "churn_probability")
                )
                segments = list(
                    CustomerSegment.objects.filter(household_key__in=score_map).only(
                        "id", "household_key", "churn_probability"
                    )
                )
                for segment in segments:
                    segment.churn_probability = score_map[segment.household_key]
                CustomerSegment.objects.bulk_update(
                    segments, ["churn_probability"], batch_size=1000
                )
        return JsonResponse(
            {
                "success": True,
        ...
        return JsonResponse(
            {"success": False, "error": f"Churn training failed: {exc}"}, status=500
        )
...
def activate_churn_experiment(request, experiment_id):
    ...
    return JsonResponse(
        {
            "success": True,
            "message": "The selected experiment is now the active dashboard model.",
        }
    )
...
def cache_churn_experiment_history(request, experiment_id):
        ...
        return JsonResponse(
            {
                "success": True,
                "already_cached": True,
                "window_records": experiment.window_cache.customer_windows.count(),
    ...
    try:
        config = ChurnWindowConfig(
            ...
            (
                None
                if experiment.method == WindowMethod.NON_OVERLAPPING.value
                else experiment.step_size_days
            ),
        )
        ...
        legacy_predictions = pd.DataFrame.from_records(
            CustomerChurnPrediction.objects.filter(
                ...
                "snapshot__household_key",
                "snapshot__cutoff_day",
                "snapshot__observation_window_days",
                "churn_probability",
            )
        )
        if not legacy_predictions.empty:
            legacy_predictions = legacy_predictions.rename(
                columns={
        ...
        if not direct_predictions.empty:
            direct_predictions["observation_window_days"] = (
                experiment.observation_window_days
            )
            legacy_predictions = pd.concat(
                [legacy_predictions, direct_predictions], ignore_index=True
            ).drop_duplicates(["household_key", "cutoff_day"], keep="first")
        with db_transaction.atomic():
            experiment.window_cache = window_cache
            experiment.save(update_fields=["window_cache"])
            stats["scored_records"] = _persist_cache_predictions(
                experiment, window_cache, legacy_predictions
            )
        ...
        return JsonResponse(
            {"success": False, "error": f"Could not build the history cache: {exc}"},
            status=500,
        )
...
def churn_api(request):
    if request.method == "POST":
        ...
        metrics = {
            ...
            "avg_txns": qs.aggregate(Avg("total_transactions"))[
                "total_transactions__avg"
            ]
            or 0,
            "avg_basket": qs.aggregate(Avg("avg_basket_value"))["avg_basket_value__avg"]
            or 0,
            "avg_churn_probability": qs.aggregate(Avg("churn_probability"))[
                "churn_probability__avg"
            ]
            or 0,
            "max_churn_probability": qs.aggregate(Max("churn_probability"))[
                "churn_probability__max"
            ]
            or 0,
            "min_churn_probability": qs.aggregate(Min("churn_probability"))[
                "churn_probability__min"
            ]
            or 0,
        }
