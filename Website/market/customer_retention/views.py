...
def retention_dashboard(request):
    ...
    try:
        minimum_probability = min(
            max(float(request.GET.get("minimum_probability", 0.50)), 0), 1
        )
    ...
    try:
        maximum_customers = (
            int(maximum_customers_raw) if maximum_customers_raw else None
        )
    ...
    customers = CustomerSegment.objects.filter(
        churn_probability__gte=minimum_probability
    ).annotate(
        churn_percentage=ExpressionWrapper(
            F("churn_probability") * 100.0, output_field=FloatField()
        )
    )
    if customer_query:
    ...
    churn_ordering = (
        "churn_probability"
        if probability_order == "ascending"
    ...
    return render(
        request,
        "site/customer_retention/customer_retention.html",
        {
            "page_obj": page_obj,
