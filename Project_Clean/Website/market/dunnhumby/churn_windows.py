...
def _load_transactions() -> pd.DataFrame:
    ...
    products = pd.DataFrame.from_records(
        DunnhumbyProduct.objects.values("product_id", "department", "commodity_desc")
    )
...
def build_customer_features(
    ...
    lifetime_transactions = lifetime_transactions[
        lifetime_transactions["day"] <= cutoff_day
    ].copy()
    if lifetime_transactions.empty:
        return pd.DataFrame()
    observation = observation.copy()
    observation["discount_value"] = (
        observation[["retail_disc", "coupon_disc", "coupon_match_disc"]]
        .abs()
        .sum(axis=1)
    )
    eligible_customers = pd.Index(
        lifetime_transactions["household_key"].unique(), name="household_key"
    )
    window_aggregations = {
    ...
    basket_values = observation.groupby(["household_key", "basket_id"])[
        "sales_value"
    ].sum()
    base["average_basket_value"] = basket_values.groupby(level=0).mean()
    lifetime = (
        lifetime_transactions.groupby("household_key")
        .agg(
    ...
    lifetime_basket_values = lifetime_transactions.groupby(
    ...
    base["lifetime_average_basket_value"] = lifetime_basket_values.groupby(
        level=0
    ).mean()
    base["discount_ratio"] = base["total_discount"] / base[
        "total_sales_value"
    ].abs().replace(0, np.nan)
    coupon_use = (
        observation.loc[
            (observation["coupon_disc"] != 0) | (observation["coupon_match_disc"] != 0)
        ]
        .groupby("household_key")["basket_id"]
        .nunique()
    )
    base["coupon_usage_count"] = coupon_use
    base["coupon_usage_rate"] = base["coupon_usage_count"] / base["frequency"].replace(
        0, np.nan
    )

    purchase_days = (
        observation[["household_key", "day"]]
        .drop_duplicates()
        .sort_values(["household_key", "day"])
    )
    purchase_days["gap"] = purchase_days.groupby("household_key")["day"].diff()
    gaps = (
        purchase_days.groupby("household_key")["gap"]
    ...
    latest_purchase_days = (
        purchase_days.groupby("household_key").tail(1).set_index("household_key")
    )
    base["days_since_previous_purchase"] = latest_purchase_days["gap"]

    if observation_start is None:
        observation_start = (
            int(observation["day"].min()) if not observation.empty else cutoff_day
        )
    midpoint = observation_start + (cutoff_day - observation_start + 1) // 2
    old = (
        observation[observation["day"] < midpoint]
        .groupby("household_key")
        .agg(
            old_frequency=("basket_id", "nunique"),
            old_monetary=("sales_value", "sum"),
            old_basket=("basket_id", "nunique"),
        )
    )
    recent = (
        observation[observation["day"] >= midpoint]
        .groupby("household_key")
        .agg(
            recent_frequency=("basket_id", "nunique"),
            recent_monetary=("sales_value", "sum"),
            recent_basket=("basket_id", "nunique"),
        )
    )
    ...
    base["frequency_change_ratio"] = base["frequency_change"] / base[
        "old_frequency"
    ].replace(0, np.nan)
    base["monetary_change_ratio"] = base["monetary_change"] / base[
        "old_monetary"
    ].replace(0, np.nan)
    base = base.drop(
        columns=[
...
def build_training_dataset(
    transactions: pd.DataFrame, config: ChurnWindowConfig
) -> pd.DataFrame:
    ...
    for window in generate_time_windows(
        transactions.day.min(), transactions.day.max(), config
    ):
        observation_left = np.searchsorted(
            transaction_days, window["observation_start"], side="left"
        )
        observation_right = np.searchsorted(
            transaction_days, window["observation_end"], side="right"
        )
        observed = transactions.iloc[observation_left:observation_right]
        lifetime_right = np.searchsorted(
            transaction_days, window["cutoff_day"], side="right"
        )
        ...
        if features.empty:
            continue
        label_left = np.searchsorted(
            transaction_days, window["label_start"], side="left"
        )
        label_right = np.searchsorted(
            transaction_days, window["label_end"], side="right"
        )
...
CLASSIFICATION_THRESHOLD_CANDIDATES = tuple(
...
def experiment_metadata() -> dict:
    ...
    return {
        ...
        "time_split": (
            "Purged chronological split: cutoff dates are allocated oldest 70% "
            "for training, next 15% for validation, and newest 15% for testing. "
            "Rows whose label horizon reaches a later split are excluded."
        ),
        "threshold_selection": {
...
def _select_classification_threshold(y_true, probabilities) -> float:
    ...
    for threshold in CLASSIFICATION_THRESHOLD_CANDIDATES:
        ...
        if candidate_f1 > best_f1 or (
            candidate_f1 == best_f1 and candidate_recall > best_recall
        ):
...
def train_and_score(config: ChurnWindowConfig, cached_training_dataset=None):
    ...
    if cached_training_dataset is None:
        ...
        complete_windows = list(
            generate_time_windows(bounds["min_day"], bounds["max_day"], config)
        )
    ...
    if transactions.empty:
        raise ValueError("No transaction data is available for churn training.")
    dataset = (
        cached_training_dataset.copy()
        if cached_training_dataset is not None
        else build_training_dataset(transactions, config)
    )
    if dataset.empty or dataset.cutoff_day.nunique() < 3:
        raise ValueError(
            "Not enough complete time windows to create train, validation, and test periods."
        )
    cutoffs = sorted(dataset.cutoff_day.unique())
    train_end, validation_end = int(len(cutoffs) * 0.70), int(len(cutoffs) * 0.85)
    train_end = max(1, train_end)
    validation_end = max(train_end + 1, validation_end)
    if validation_end >= len(cutoffs):
        validation_end = len(cutoffs) - 1
    train_cutoffs = cutoffs[:train_end]
    validation_cutoffs = cutoffs[train_end:validation_end]
    test_cutoffs = cutoffs[validation_end:]

    if not validation_cutoffs or not test_cutoffs:
        raise ValueError(
            "Not enough cutoff dates for a time-based validation and test split."
        )
    excluded = {
        "household_key",
        "is_churn",
    ...
    encoded = pd.get_dummies(
        dataset[feature_columns], columns=["rfm_segment"], dtype=float
    )
    first_validation_cutoff = validation_cutoffs[0]
    first_test_cutoff = test_cutoffs[0]

    train_mask = (
        dataset.cutoff_day.isin(train_cutoffs)
        & (dataset.label_end <= first_validation_cutoff)
    )
    validation_mask = (
        dataset.cutoff_day.isin(validation_cutoffs)
        & (dataset.label_end <= first_test_cutoff)
    )
    test_mask = dataset.cutoff_day.isin(test_cutoffs)

    if not train_mask.any() or not validation_mask.any() or not test_mask.any():
        raise ValueError(
            "The purged time split produced an empty train, validation, or test set. "
            "Use a longer history or less-overlapping window settings."
        )
    model = _new_model()
    ...
    cutoff_blocks = (
        np.array_split(prediction_cutoffs, min(5, len(prediction_cutoffs)))
        if len(prediction_cutoffs)
        else []
    )
    for cutoff_block in cutoff_blocks:
        ...
        history_mask = dataset.label_end <= block_start
        ...
        fold = dataset.loc[
            cutoff_mask, ["household_key", "cutoff_day", "observation_window_days"]
        ].copy()
        fold["churn_probability"] = fold_model.predict_proba(encoded.loc[cutoff_mask])[
            :, 1
        ]
    ...
    current_snapshots = current_features[
        [column for column in snapshot_columns if column != "is_churn"]
    ].copy()
    return (
        metrics,
        current_features[["household_key", "churn_probability"]],
